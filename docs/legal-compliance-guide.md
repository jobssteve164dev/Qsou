...
> 文档定位：数据源接入和数据使用的合规检查参考，不构成法律意见，也不能单独证明某个来源已经获得访问、保存、处理或分发授权。数据资产边界见[自主数据资产设计指导](./data-sovereignty-design-guidelines.md)。

## 0. 来源权利登记

每个正式数据源在接入前应分别确认并记录：自动访问、保存原文与附件、生成摘要或模型派生、内部展示、导出及再分发的允许边界。公开可访问不等于拥有版权或再分发权；条款不明确时应限制用途并进入人工复核，而不是默认放行。
...
- **白名单策略**: 仅自动采集已确认允许程序访问的数据源；公开可访问本身不构成放行依据
- **风险评估**: 每个新增数据源都需要进行法律风险评估
- **运行门禁**: 自动调度和人工触发都必须校验登记的自动访问权利状态；未明确允许时保持关闭，不能以“公开可访问”替代授权判断
...
### 3.2 请求头规范
- **准确标识客户端**: 使用稳定且可联系的采集身份，不伪装浏览器或绕过访问控制
- **尊重缓存**: 正确处理HTTP缓存机制
- **条件请求**: 支持If-Modified-Since等条件请求头

### 3.3 错误处理与重试
- **优雅降级**: 遇到403/429错误时立即停止访问该域名
- **退避算法**: 仅对临时网络故障和服务端错误实施有界退避；403/429不自动重试
- **错误记录**: 详细记录所有访问异常以便审查
