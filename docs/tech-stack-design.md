...
> 文档定位：历史技术选型与实现参考，不代表当前完成状态，也不定义数据权威。项目方向、存储分层与验收边界以[自主数据资产设计指导](./data-sovereignty-design-guidelines.md)为准。
...
#### 原始证据归档（目标能力）
- **核心功能**: 在解析和索引前保存原始响应、文件、采集上下文、时间与内容哈希
- **存储原则**: 使用适合长期保存和开放导出的对象存储或文件归档
- **权威边界**: 原始证据与来源登记是事实权威；不得由 Elasticsearch、Qdrant 或模型结果反向定义
- **当前状态**: 本文不指定具体产品；落地前需先完成数据模型、回放和恢复验收设计

#### Elasticsearch 8.x
- **核心功能**: 可从标准文档重建的全文搜索与聚合索引
- **索引策略**:
  ```json
  {
    "mappings": {
